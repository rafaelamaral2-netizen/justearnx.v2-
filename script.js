...
async function loadProfile(userId) {
  try {
    ...
    if (result.error) {
      console.error("loadProfile:", result.error.message);
      state.profile = null;
      return;
    }
    ...
    console.error("loadProfile fatal:", err);
    state.profile = null;
  }
}
...
async function loadCreators() {
  try {
    ...
    if (result.error) {
      console.error("loadCreators:", result.error.message);
      state.creators = [];
      return;
    }
    ...
    console.error("loadCreators fatal:", err);
    state.creators = [];
  }
}
  ...
  return (
    '<div id="auth-form">' +
      ...
      '<div class="field">' +
        ...
        '<input id="a-pass" type="password" placeholder="â¢â¢â¢â¢â¢â¢â¢â¢" autocomplete="current-password" />' +
      '</div>' +
...
function bindAuth() {
  ...
  submit.addEventListener("click", async () => {
    ...
    try {
      ...
      console.error("bindAuth error:", err);
      renderAuthOnly();
    } finally {
...
function renderHome() {
  ...
  const name = (profile && (profile.display_name || profile.username)) ? (profile.display_name || profile.username) : "Creator";
  ...
  return (
    '<div class="page">' +
      ...
      '</div>' +
      '<div class="stats-row">' +
        '<div class="stat-card"><div class="stat-value accent">$' + escapeHtml(Number(((profile && profile.earnings_total) || 0)).toFixed(0)) + '</div><div class="stat-label">Earnings</div></div>' +
        '<div class="stat-card"><div class="stat-value">' + escapeHtml((profile && profile.followers_count) || 0) + '</div><div class="stat-label">Followers</div></div>' +
        '<div class="stat-card"><div class="stat-value">' + escapeHtml((profile && profile.posts_count) || 0) + '</div><div class="stat-label">Posts</div></div>' +
      '</div>' +
      ...
      '<div class="market-grid">' +
        (
          top.length === 0
            ? '<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">â</div><div class="empty-title">No ranking yet</div><div class="empty-desc">Add stats columns like followers_count, posts_count, earnings_total, engagement_score to see a real leaderboard.</div></div>'
            : top.map(item => {
...
function renderProfile() {
  ...
  const name = (user && (user.display_name || user.username)) ? (user.display_name || user.username) : "You";
  const handle = user && user.username ? "@" + user.username : (user && user.email ? user.email : "");
  const bio = user && user.bio ? user.bio : "No bio yet.";
  const coverInner = user && user.cover_url
  ...
  return (
    '<div class="page" style="padding-top:0">' +
      ...
      '<div class="profile-info">' +
        ...
        (user && user.verified ? '<div class="chip" style="margin-top:8px;">' + icoCheck() + ' Verified</div>' : '') +
        '<div class="profile-meta">' +
          (user && user.country ? '<div class="profile-meta-item">' + icoGlobe() + escapeHtml(user.country) + '</div>' : '') +
          (user && user.category ? '<div class="profile-meta-item">' + icoTag() + escapeHtml(user.category) + '</div>' : '') +
        '</div>' +
        '<div class="profile-bio">' + escapeHtml(bio) + '</div>' +
        '<div class="profile-stats">' +
          '<div class="profile-stat"><div class="profile-stat-val">' + escapeHtml((user && user.posts_count) || 0) + '</div><div class="profile-stat-label">Posts</div></div>' +
          '<div class="profile-stat"><div class="profile-stat-val">' + escapeHtml((user && user.followers_count) || 0) + '</div><div class="profile-stat-label">Followers</div></div>' +
          '<div class="profile-stat"><div class="profile-stat-val">$' + escapeHtml(Number(((user && user.earnings_total) || 0)).toFixed(0)) + '</div><div class="profile-stat-label">Earned</div></div>' +
        '</div>' +
...
function renderWallet() {
  const earnings = Number(((state.profile && state.profile.earnings_total) || 0));
...
function renderAvatarButton(profile) {
  const initials = getInitials(profile && (profile.display_name || profile.username || profile.email || "U"));
